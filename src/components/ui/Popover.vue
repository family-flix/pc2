...
const contentClassName = cn(
  ...
  "relative rounded-md bg-w-bg-0 text-w-fg-1 shadow-lg focus:shadow-md focus:ring-2 focus:ring-violet-700",
  className
);
</script>
    ...
    <PopoverPrimitivePortal :store="store" class-name="">
      <PopoverPrimitiveContent :store="store" :class-name="contentClassName">
        ...
        <PopoverPrimitiveClose
          class-name="font-inherit rounded-full h-6 w-6 inline-flex items-center justify-center absolute top-3 right-3"
          :store="store"
        >
          <X class="w-8 h-8"></X>
        </PopoverPrimitiveClose>
